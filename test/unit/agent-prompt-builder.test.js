...
function run() {
    ...
    assert.ok(payload.threadContext.replyChainMessages.some((message) => message.messageId === 'msg_2'))
    assert.strictEqual(payload.socialPolicy.mode, 'quiet')
    ...
    assert.ok(Array.isArray(relevancePayload.threadContext.currentThread.messages))
    assert.ok(Array.isArray(relevancePayload.threadContext.ambientRecentMessages))
