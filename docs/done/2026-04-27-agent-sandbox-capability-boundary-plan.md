...
> 当前决策：独立 `agent-sandbox` 和完整浏览器自动化已放弃作为当前目标。现阶段保留主进程内受限只读浏览器工具（公开网页读取、搜索、截图）以及现有 SSRF / 内网 / 凭证 URL 防护；本文档作为历史备选方案和后续重新评估时的参考，不再作为当前验收目标。
...
历史备选方向是：主 Bot 进程保持最小权限，危险或不确定能力放进隔离沙箱执行。Agent 可以获得更强的浏览器和临时工作能力，但不能越过权限、确认、审计和资源边界。该方向当前不继续推进。
...
以下内容仅作为后续重新启用独立沙箱时的设计参考。

### Phase S1：历史备选：沙箱替换现有浏览器工具
