...
- Phase 3：记忆管理页，允许 root 查看、筛选、删除和按筛选清理记忆条目。
- Phase 4：可配置 persona、每群 Agent 开关、冷却参数、工具权限策略和受限工具审批状态。
...
- Phase 3.3 补齐记忆质量治理：规则兜底提取、冲突覆盖、重要性评分、访问统计和自动过期。
- WebUI 提供 Agent 记忆查看、筛选、删除和按筛选清理能力。
...
当前状态：已实现长期记忆文件存储、LLM `memoryHints` 写入、规则兜底记忆提取、记忆检索注入、活跃话题低频固化、冲突覆盖、重要性评分、访问统计、自动过期、Root 记忆命令，以及 WebUI/API 的记忆查看、筛选、删除和按筛选清理。
...
当前状态：大部分完成。Docker 本地构建启动、QQ 群真实消息、@Bot、普通自然语言、记忆、工具确认、上下文追问和群管工具已多轮验证；仍需补齐 README 使用说明、最终 QQ 实测矩阵和文档化安全边界。

### Phase 7：工具结果闭环与领域工具

目标：让 Agent 不只是触发工具，而是能基于工具结果生成自然语言最终回复，并优先扩展 B 站和 Bot 配置相关领域工具。

当前状态：已实现。

- `bili.user_lookup`：按 UID 或关键词查询 B 站用户候选。
- `bili.video_lookup`：按 BV/av 查询 B 站视频基础信息。
- `bili.subscription_status`：查询本群是否订阅指定用户或番剧。
- `agent.get_group_config`：查询当前群 Agent 配置状态。
- 工具执行结果已支持回灌 LLM 生成最终群聊回复。
- Dashboard 决策页可观测工具计划、确认、执行结果和最终回复。

### Phase 8：QQ 管理、受限浏览器和自学习

目标：在权限和确认边界内扩展 QQ 群管理、QQ 账号管理、只读网页读取和显式学习记忆。

当前状态：已实现，仍需按 QQ 实测矩阵补齐最终验证记录。

- QQ 群只读工具：群信息、禁言列表、精华消息、公告、系统消息、忽略通知、@全体剩余次数。
- QQ 成员工具：成员信息、成员搜索、禁言、解禁、踢人、设置群名片。
- QQ 消息/群状态工具：全员禁言、撤回消息、设置/取消精华消息。
- QQ 审批工具：待处理加群申请查询、同意/拒绝加群申请、好友申请查询和处理。
- QQ 账号工具：设置 Bot 在线状态、向指定用户设置输入状态。
...
当前 Phase 1-8 已经落地，Phase 6 的最终文档和实测矩阵仍需收口。下一步不应继续堆新工具，而应进入 Runtime V2：抽 `AgentRunner`、升级 `ToolSpec`、拆分 guardrails、升级 trace span，并把上下文和记忆治理模块化。
...
下一阶段建议定义为 Phase 9：`Documentation + Runtime V2 Preparation`。

目标：

- 补齐 README、QQ 实测矩阵和安全边界说明。
- 为 Runtime V2 的 `AgentRunner`、`ToolSpec V2`、guardrails、trace span 和上下文治理做实施准备。

验收：

- 用户能按文档开启、配置、测试和回滚 Agent。
- QQ 实测矩阵覆盖 @Bot、回复 Bot、普通自然语言、记忆、订阅、QQ 群管理、审批、越权拒绝和短码确认。
- Runtime V2 的开发顺序和验收标准明确记录在 `docs/plans/2026-04-26-agent-runtime-v2-roadmap.md`。
